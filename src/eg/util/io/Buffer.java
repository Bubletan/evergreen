...
/**
 * A buffer that allows input and output, bit access system and
 * a cipher that can be bound to encrypt the packet opcodes.<br>
 * <br>
 * By default the data is put and got in a big-endian order and got as signed values.
 * Using the optional methods, it is possible to use different byte orders and get unsigned values.
 * @author Rob / Bubletan
 */
public final class Buffer {
	...
	/**
	 * Initializes a new {@link Buffer} by allocating space of the default capacity (16).
	...
	/**
	 * Initializes a new {@link Buffer} by allocating space of the required capacity.
	...
	public Buffer(byte[] data) {
		if (data == null) {
			...
			throw new IllegalArgumentException("Data capacity must not be zero.");
		}
		this.data = data;
	}
	...
	/**
	 * Shifts the current position based on the amount of bytes
	 * and gets the new position as the index of the byte.
	 */
	...
	/**
	 * Shifts the current position based on the amount of bytes
	 * and gets the old position as the index of the byte.
	 */
	...
	/**
	 * Shifts the current position based on the amount of bits
	 * and gets the new position as the index of the bit.
	 */
	...
	/**
	 * Shifts the current position based on the amount of bits
	 * and gets the old position as the index of the bit.
	 */
	...
	/**
	 * Gets the data as a modifiable {@code byte[]}.
	 * Replacing an element in the returned array will also replace it in the buffer.
	 * Alternatively use {@link #getUnmodifiableData()} or {@link #getShrunkUnmodifiableData()}.
	 */
	public byte[] getData() {
		return data;
	}
	...
	/**
	 * Gets the data as an unmodifiable {@code byte[]}.
	 * Alternatively use {@link #getData()} or {@link #getShrunkUnmodifiableData()}.
	 */
	...
	/**
	 * Gets the data as an unmodifiable {@code byte[]}
	 * which is shrunk to a size based on the current position.
	 * Alternatively use {@link #getData()} or {@link #getUnmodifiableData()}.
	 */
	 ...
	 * Expands the buffer capacity when needed if automatic expanding is enabled.
	 */
	...
	/**
	 * Begins a block with an unsigned 8 bit size variable header with an opposite sign.<br>
	 * Returns itself to allow method chaining.
	 * @see #endBlock()
	 */
	...
	public Buffer putBytes(byte[] src, int offset, int length) {
		...
		if (offset + length > src.length) {
			throw new ArrayIndexOutOfBoundsException("Source array is not big enough: " + (offset + length));
		}
	...
	public int getMedium() {
		...
		pos += 3;
		int tmp = ((data[pos - 3] & 0xff) << 16) + ((data[pos - 2] & 0xff) << 8) + (data[pos - 1] & 0xff);
		if (tmp > 0x7fffff) {
			tmp -= 0x1000000;
		}
		return tmp;
	...
	public int getLEMedium() {
		...
		pos += 3;
		int tmp = ((data[pos - 1] & 0xff) << 16) + ((data[pos - 2] & 0xff) << 8) + (data[pos - 3] & 0xff);
		if (tmp > 0x7fffff) {
			tmp -= 0x1000000;
		}
		return tmp;
	...
	public int getLEInt() {
		...
		pos += 4;
		return ((data[pos - 1] & 0xff) << 24) + ((data[pos - 2] & 0xff) << 16) + ((data[pos - 3] & 0xff) << 8) + (data[pos - 4] & 0xff);
	}
	...
	/**
	 * Gets a {@code String} created from bytes ending with a line break character (10).
	 * @see #putLine(String)
	 */
	...
	/**
	 * Gets a {@code String} created from bytes ending with a null character (0).
	 ...
	 * If destination array is {@code null},
	 * it is automatically created with a length that is able to store the required bytes.
	...
	/**
	 * Gets a {@code byte[]} as an array copy in a reversed byte order.
	 * If destination array is {@code null},
	 * it is automatically created with a length that is able to store the required bytes.
	...
	public Buffer putBits(int n, int value) {
		...
		if (!hasBitPuttingCapacity(n)) {
			throw new ArrayIndexOutOfBoundsException("Buffer overflow: " + ((pos + n - 1) >> 3));
		}
        int bytepos = pos >> 3;
        int bitoff = 8 - (pos & 7);
        pos += n;
