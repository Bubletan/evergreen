...
public final class GamePacketDecoder extends MessageToMessageDecoder<GamePacket> {
    ...
    private static final AbstractGamePacketDecoder<?>[] DECODERS = new AbstractGamePacketDecoder<?>[256];
    
    static {
        int successCount = 0, totalCount = 0;
        Map<Class<? extends AbstractGamePacketDecoder<?>>, AbstractGamePacketDecoder<?>> pool = new HashMap<>();
        for (InboundAbstractGamePacketDeclaration dec : InboundAbstractGamePacketDeclaration.values()) {
            try {
                AbstractGamePacketDecoder<?> decoder = pool.get(dec
                        .getDecoderClass());
                if (decoder == null) {
        ...
        (successCount == totalCount ? System.out : System.err)
                .println("Registered succesfully " + successCount + " out of "
                        + totalCount + " packet decoders.");
    }
